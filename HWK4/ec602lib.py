...
def code_analysis_py(prog):
    ...
    try:
        filehand = open(prog)
        processed_tokens = []
        for tok in tokenize.generate_tokens(filehand.readline):
            t_type = tok[0]
        ...
        src = "\n".join(
            x for x in tokenize.untokenize(processed_tokens).splitlines()
            if x.strip() and x != "\\")
    except:
        src = ""
    finally:
...
def overallpy(program_name, testclass, refcode, orig_program=None):
    ...
    code_metrics = code_analysis_py(program_name)
    retstr += code_size_report(code_metrics, refcode)
...
def pyshell(Parms):
      ...
      Parms['gradesummary']=gradesummary
